...
export function composeThreeDistinctDailyShakes(options: ComposeOptions = {}): Shake[] {
  // The target is supplied by the dynamic calorie/planning engine.
  // Never replace it with a fixed daily-calorie floor. If an older caller omits
  // targetCalories, derive it from the user's calculated profile target.
  const explicitTarget = Number(options.targetCalories);
  const profileTarget = Number(
    options.userProfile?.calorieGoal ||
    options.userProfile?.maintenanceCalories ||
    0
  );
  const targetKcal =
    Number.isFinite(explicitTarget) && explicitTarget > 0
      ? Math.round(explicitTarget)
      : Math.round(profileTarget);
