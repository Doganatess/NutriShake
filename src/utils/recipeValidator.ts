...
export function validateAndSanitizeShake(
  ...
  if (stock && Object.keys(stock).length > 0) {
    ...
    // b) Stok hiçbir zaman aşılmamalıdır. Eksik miktarı sessizce kabul etmek yerine
    // mevcut miktarı kullan; aşağıdaki kalori tamamlama adımı farkı başka uygun
    // stok malzemeleriyle kapatmaya çalışır.
    if (!stockValidation.isValid) {
      for (const missingItem of stockValidation.missing) {
        const itemIdx = workingIngredients.findIndex(
          (i) => canonicalIngredientId(i.ingredientId) === canonicalIngredientId(missingItem.ingredientId)
  ...
  if (stock && Object.keys(stock).length > 0) {
    ...
    if (!finalValidation.isValid) {
      errors.push('Tarif mevcut stok miktarlarını aşamaz; stok dışı miktar oluşturulması engellendi.');
    }
  }
